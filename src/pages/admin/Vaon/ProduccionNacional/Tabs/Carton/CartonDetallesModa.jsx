...
export default function PoliticaDetallesModal({
  open,
  onClose,
  fetchById,
  id,
  ...
  const handleDownload = async () => {
    try {
      const res = await sevices.DownloadDocumento(id);
      ...
      <div className="relative z-10 w-[92%] max-w-7xl rounded-2xl bg-white shadow-xl ring-1 ring-slate-200 max-h-[calc(100vh-2rem)] overflow-y-auto">
        ...
        <div className="px-5 py-5">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-12">
              <div className="md:col-span-2">
                ...
                <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800">
                  {data?.periodo || 'Sin fecha'}
                </div>
              </div>

              <div className="md:col-span-7">
                <label className="mb-1 block text-sm font-semibold text-slate-700">
                  Costo unitario caja [Bs]
                </label>
                <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800">
                  {data?.costo_unitario_caja || 'Sin título'}
                </div>
              </div>
              ...
              <div className="md:col-span-3">
                ...
                <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800">
                  {data?.metros_cuadrados_caja || 'Sin código'}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <PdfPreviewTW documentoId={id} getDocument={sevices.viewDocumeto} />
          </div>
        </div>
