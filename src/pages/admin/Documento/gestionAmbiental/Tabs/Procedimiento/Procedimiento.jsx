...
import { ProcesosServices } from '@service/Documentos/gestionAmbiental/Procedimiento.js';
import ProcedimientoModa from './ProcedimientoModa';
import ProcedimientoDetallesModal from './ProcedimientoDetallesModal';
...
export default function Politica() {
  ...
  const hanldeDelete = async () => {
    setLoading(true);
    try {
      const res = await ProcesosServices.delete(idRow);
      if (res.ok) {
  ...
  const handleUpdate = async () => {
    try {
      setLoading(true);
      const res = await ProcesosServices.update(idRow, payloadUpdate);
      if (res.ok) {
  ...
  const hanldeCreate = async () => {
    try {
      setLoading(true);
      const res = await ProcesosServices.create(payloadCreate);
      if (res.ok) {
        toast.success('Registro actualizado con éxito');
  ...
  return (
    <>
      <TablaRetutilizable
        ref={tableRef}
        getObj={ProcesosServices.getAll}
        titulo="Gestion de ambiental/ Procedimiento"
      ...
      <ProcedimientoModa
        ...
        fetchById={ProcesosServices.getId}
        id={idRow}
        isEditing={true}
      />
      <ConfirmModal
