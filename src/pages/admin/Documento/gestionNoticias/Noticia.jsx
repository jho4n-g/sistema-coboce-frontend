...
import NoticiaDetallesModal from './NoticiaDetallesModal';
...
export default function Politica() {
  ...
  const hanldeDelete = async () => {
    setLoading(true);
    try {
      const res = await NoticiaServices.delete(idRow);
      if (res.ok) {
  ...
  const handleUpdate = async () => {
    try {
      setLoading(true);
      const res = await NoticiaServices.update(idRow, payloadUpdate);
      if (res.ok) {
  ...
  const hanldeCreate = async () => {
    try {
      setLoading(true);
      const res = await NoticiaServices.create(payloadCreate);
      if (res.ok) {
        toast.success('Registro actualizado con éxito');
  ...
  return (
    <>
      <TablaRetutilizable
        ref={tableRef}
        getObj={NoticiaServices.getAll}
        titulo="Gestion de noticias"
      ...
      <NoticiaModal
        ...
        fetchById={NoticiaServices.getId}
        id={idRow}
        isEditing={true}
      />
      <ConfirmModal
